...
if [ -e fdroid-statuspage-deployment ]; then
  yml=fdroid-statuspage-deployment/group_vars/all.yml
  ...
  done < <( yq -r < "$yml" '.mirrors[] | select(.official) | .base_url' | sort )
  unofficial_mirrors=() unofficial_onion_mirrors=()
  while read -r; do
    if [[ "$REPLY" == *.onion* ]]; then
      unofficial_onion_mirrors+=( "$REPLY" )
    else
      unofficial_mirrors+=( "$REPLY" )
    fi
  done < <( yq -r < "$yml" '.mirrors[] | select(.official | not) | .base_url' | sort )
else
  mirrors=(
    https://f-droid.org
